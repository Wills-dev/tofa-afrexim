...
export const useGetAllSupervisorCompanies = () => {
  const {
    loading,
    setLoading,
    currentPage,
  ...
  const [allSupervisorCompanies, setAllCompanies] = useState<CompanyType[]>([]);

  const getAllSupervisorCompanies = async () => {
    try {
      let url = `/agent/supervisor/companies?page=${currentPage}&limit=${limit}${
